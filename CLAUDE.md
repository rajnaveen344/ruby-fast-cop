...
**Status:** 9,399 / 11,217 (84%) corrections wired. 1,818 expected corrections across ~115 cops still unwired. Per-cop counts in `.correction_worklist.txt`. Per-dept totals in `COPS.md` summary.
...
- **Cluster: Layout brace-layout** (this commit) — 3 cops, +49 corrections via shared helper `multiline_literal_brace_layout.rs`. MultilineMethodCall/Array/Hash BraceLayout. `new_line` → insert `\n`; `same_line` → replace whitespace+brace; comment-aware atomic move; heredoc-chain detection in MethodCall variant.
