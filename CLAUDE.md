...
**Status:** 9,217 / 11,217 (82%) corrections wired. 2,000 expected corrections across ~125 cops still unwired. Per-cop counts in `.correction_worklist.txt`. Per-dept totals in `COPS.md` summary.
...
- **Cluster: Layout empty-line** (this commit) — 3 cops, +55 Layout corrections. EmptyLineBetweenDefs (~25): build_correction with newline_pos detection (handles `;`-on-same-line via `begin_pos - 1` fallback). EmptyLineAfterGuardClause (~18): insert `\n` after last line; rubocop:enable / :nocov: directive-aware. EmptyLinesAroundAccessModifier (~12): block-body-empty-lines cross-cop config gates around-block fixes.
