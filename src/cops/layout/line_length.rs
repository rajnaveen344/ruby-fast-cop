...
mod tests {
    ...
    fn check(source: &str) -> Vec<Offense> { check_with_max(source, 80) }

    #[test]
    fn allows_short_lines() {
        assert_eq!(check("puts 'hello'").len(), 0);
    }

    #[test]
    fn allows_exactly_max_length() {
        assert_eq!(check(&"x".repeat(80)).len(), 0);
    }

    #[test]
    fn detects_line_over_max() {
        let offenses = check(&"x".repeat(81));
        assert_eq!(offenses.len(), 1);
        assert_eq!(offenses[0].location.line, 1);
        assert_eq!(offenses[0].location.column, 80);
    ...
    fn respects_custom_max() {
        assert_eq!(check_with_max(&"x".repeat(100), 80).len(), 1);
        assert_eq!(check_with_max(&"x".repeat(100), 160).len(), 0);
    }
    ...
    fn counts_unicode_correctly() {
        assert_eq!(check(&"🎉".repeat(80)).len(), 0);
        assert_eq!(check(&"🎉".repeat(81)).len(), 1);
    }

    #[test]
    fn ignores_shebang() {
        let source = format!("#!/usr/bin/env ruby {}\nputs 'ok'", "x".repeat(100));
        assert_eq!(check(&source).len(), 0);
    }

    #[test]
    fn ignores_lines_with_uri() {
        let source = format!("# See: https://example.com/very/long/path/to/resource{}", "/x".repeat(50));
        assert_eq!(check(&source).len(), 0);
    }

    #[test]
    fn message_format_matches_rubocop() {
        assert_eq!(check(&"x".repeat(100))[0].message, "Line is too long. [100/80]");
    }
